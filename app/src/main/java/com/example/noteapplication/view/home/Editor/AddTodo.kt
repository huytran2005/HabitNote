...
@Composable
fun TitleContent(title: MutableState<String>) {
    OutlinedTextField(
        value = title.value,
...
fun AddTodoScreen(
    ...
    Column(
        ...
        TitleAddNote(title = "Add To-Do", navController = navController)
        TitleContent(title)
        DynamicToDoInput(todoList)
    }
    ...
    Box(
        ...
        ComfirmContent(
            onClick = {
                if (title.value.isNotEmpty() and todoList.isNotEmpty()){
                viewModel.saveNote(
                    Note(
                        title = title.value,
...
@Composable
fun DynamicToDoInput(todoList: SnapshotStateList<Todo>) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(top = 12.dp),
